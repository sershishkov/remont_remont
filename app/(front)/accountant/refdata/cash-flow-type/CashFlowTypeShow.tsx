...
const headerFields = ['Наименование', '+ или -'];

const tableFields = ['cashFlowTypeName', 'incomeOrExpense'];
...
const arrToShow = (enteredArr: any) => {
  ...
  const transformedArr = localArr.map((currentItem: any) => {
    const currentOperation = arr__incomeOrExpense.find(
      (item) => item._id === currentItem.incomeOrExpense
    );

    return {
      _id: currentItem._id,
      cashFlowTypeName: currentItem.cashFlowTypeName,

      incomeOrExpense: currentOperation?.caption,
    };
  });
  return transformedArr;
};

export default function CashFlowTypeShow({
  currentURL,
  tableHeader,
}: {
