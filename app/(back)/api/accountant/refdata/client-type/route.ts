...
export const POST = async (request: NextRequest) => {
  ...
  try {
    ...
    const already__Exists = await Model__ClientType.findOne({
      clientTypeName,
    });
    ...
    const new__ITEM = await Model__ClientType.create({
      clientTypeName,
    });
...
export const GET = async (request: NextRequest) => {
  ...
  try {
    ...
    const all__ITEMS = await Model__ClientType.find(filterObject)
      .limit(pageSize)
      .skip(skip)
      .sort({
        clientTypeName: 1,
